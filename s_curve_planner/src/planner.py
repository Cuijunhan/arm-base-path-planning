class SCurvePlanner:
    ...
    def build_normalized_s_curve(self, t4=None):
        if t4 is None:
            t4 = 1.0 - (4.0 * self.tj + 2.0 * self.tc)
        if t4 < 0:
            raise ValueError("tj 和 tc 太大导致 t4 < 0，请减小 tj 或 tc。")

        durations = np.array([self.tj, self.tc, self.tj, t4, self.tj, self.tc, self.tj], dtype=float)
        ...
        self.s_norm = s / s_end
        self.s1_norm = v / s_end
        self.s2_norm = a / s_end
        self.s3_norm = j_tau / s_end

        return self.tau, self.s_norm, self.s1_norm, self.s2_norm, self.s3_norm
